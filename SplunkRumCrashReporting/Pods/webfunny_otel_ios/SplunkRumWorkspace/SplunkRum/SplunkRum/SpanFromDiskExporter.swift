...
let MAX_CONTENT_LENGTH = 1024 * 512 * 1024
...
class SpanFromDiskExport {
    @discardableResult static func start(spanDb: SpanDb, endpoint: String) -> (() -> Void) {
        ...
        processSpans = {
            ...
            debug_log("Reuqest url: to upload spans: \(url) spans:\(spans)")
            ...
            let task = URLSession.shared.dataTask(with: req) { _, resp, error in
                ...
                if error == nil && shouldEraseSpans(resp) {
                    ...
                    loopDelayMs = 50
                    debug_log("Success to upload spans: \(error.debugDescription) req:\(req) resp:\(String(describing: resp))")
                } else {
