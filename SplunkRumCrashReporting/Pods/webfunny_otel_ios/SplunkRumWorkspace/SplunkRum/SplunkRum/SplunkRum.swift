...
let SplunkRumVersionString = "0.0.1"
...
@objc public class SplunkRum: NSObject {
    ...
    @objc class func initialize(beaconUrl: String, rumAuth: String, options: SplunkRumOptions? = nil) -> Bool {
        if !Thread.isMainThread {
            print("WebfunnyRum: Please call WebfunnyRum.initialize only on the main thread")
            return false
        }
        if initialized || initializing {
            debug_log("WebfunnyRum already initializ{ed,ing}")
            return false
        }

        if !beaconUrl.starts(with: "https:") && options?.allowInsecureBeacon != true {
            print("WebfunnyRum: beaconUrl must be https or options: allowInsecureBeacon must be true")
            return false
        }
        ...
        debug_log("WebfunnyRum.initialize")
        ...
        srInit.end()
        initialized = true
        print("WebfunnyRum.initialize() complete")
        return true
    }
    ...
    @objc internal class func create(beaconUrl: String, rumAuth: String, appName: String?, options: SplunkRumOptions) -> Bool {
        guard Thread.isMainThread else {
            print("WebfunnyRum: Please call WebfunnyRum.create only on the main thread")
            return false
        }
        if initialized || initializing {
            debug_log("WebfunnyRum already create{ed,ing}")
            return false
        }
        ...
        srInit.end()
        initialized = true
        print("WebfunnyRum.create() complete")
        return true
    ...
    /**
        Set or override  one or more global attributes; acceptable types are Int, Double, String, and Bool.  Other value types will be silently ignored.  Can only be called after WebfunnyRum.initialize()
     */
    ...
    @objc public class func setScreenName(_ name: String) {
        if !Thread.current.isMainThread {
            debug_log("WebfunnyRum.setScreenName not called from main thread: "+Thread.current.debugDescription)
            return
        }
